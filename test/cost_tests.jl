...
_cost_term = LQRCostTerminal(Qf,xf)
_cost_mt = MinTimeCost(_cost,1.6)
_cost_term_mt = MinTimeCost(_cost_term,1.6)

EE = Expansion{Float64}(n+1,m+1)
...
gradient!(grad_mt,_cost_mt,_x,_u,dt)
gradient!(grad_term_mt,_cost_term_mt,_x)
hessian!(hess_mt,_cost_mt,_x,_u,dt)
hessian!(hess_term_mt,_cost_term_mt,_x)
