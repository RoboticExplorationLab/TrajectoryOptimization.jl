...
@test width(dyn_con) == 2(n+m)
evaluate!(fVal, dyn_con, Z)
jacobian!(∇c, dyn_con, Z)
@test (@allocated evaluate!(fVal, dyn_con, Z)) == 0
@test (@allocated jacobian!(∇c, dyn_con, Z)) == 0
...
@test integration(dyn_con) == RK3 == DEFAULT_Q
