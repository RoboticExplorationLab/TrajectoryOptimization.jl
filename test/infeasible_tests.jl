...
x0 = Problems.pendulum.x0
N = Problems.pendulum.N
n = Problems.pendulum.model.n; m = Problems.pendulum.model.m
...
prob = update_problem(copy(Problems.pendulum))
copyto!(prob.X,X0)
solve!(prob,opts_altro)
@test norm(prob.X[end] - xf) < 1.0e-3

prob_resolve = update_problem(copy(Problems.pendulum))
...
prob = update_problem(copy(Problems.pendulum),constraints=copy(constraints))
...
prob_resolve = update_problem(copy(Problems.pendulum),constraints=copy(constraints))
...
prob = copy(Problems.quadrotor_maze)
solve!(prob,opts_altro)
@test max_violation(prob) < opts_al.constraint_tolerance

prob_resolve = copy(Problems.quadrotor_maze)
