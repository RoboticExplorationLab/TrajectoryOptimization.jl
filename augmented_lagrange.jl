...
function solve_al(solver::iLQR.Solver,X0::Array{Float64,2},U0::Array{Float64,2};infeasible::Bool=true)::SolverResults
    ...
    if infeasible
        _, b = infeasible_bias(solver,X0,U0) # generates n additional control input sequences that produce the desired infeasible state trajectory
    ...
    if infeasible
        ...
        results.U .= [U0; b] # augment control with additional control inputs that produce infeasible state trajectory
    else
    ...
    for k = 1:solver.opts.iterations_outerloop
        ...
        # check if maximum constraint violation satisfies criteria
