...
function init_constraint_trajectories(constraints::ProblemConstraints,n::Int,m::Int,N::Int;
    ...
    λ          = [PartedVector(T,constraints[k],:stage).+1  for k = 1:N-1]
    μ          = [PartedVector(T,constraints[k],:stage).+1  for k = 1:N-1]
    active_set = [PartedVector(T,constraints[k],:stage).+1  for k = 1:N-1]
    ...
    λ          = [C...,  PartedVector(T,constraints[N],:terminal).+1]
    μ          = [C...,  PartedVector(T,constraints[N],:terminal).+1]
    active_set = [C...,  PartedVector(T,constraints[N],:terminal).+1]
