...
"""
function backwardpass!(solver::iLQRSolver2{T,QUAD,L,O,n,m}) where {T,QUAD<:QuadratureRule,L,O,n,m}
	N = solver.N
    ...
    while k > 0
        ...
        if solver.opts.bp_reg_type == :state
			...
			mul!(Quu_reg, fdu', fdu, solver.ρ[1], 1.0)
		...
		K[k] .= Qux_reg
		d[k] .= Q.u
		LAPACK.potrs!('U', Quu_reg, K[k])
		LAPACK.potrs!('U', Quu_reg, d[k])
		K[k] .*= -1
		d[k] .*= -1
