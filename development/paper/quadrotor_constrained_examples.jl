...
plot(log.(stats_mintime["c_max"]),title="Quadrotor Obstacle Avoidance (Minimum Time)",xlabel="iteration",ylabel="log(max constraint violation)",label="")
y = range(minimum(log.(stats_mintime["c_max"])), stop=maximum(log.(stats_mintime["c_max"])),length=100)
