...
plot_trajectory!(to_array(res.X),width=2,color=:blue,label="iLQR",aspect_ratio=:equal,xlim=[-1,11],ylim=[-1,11])
plot_trajectory!(to_array(res_inf.X),width=2,color=:purple,label="Infeasible",aspect_ratio=:equal,xlim=[-1,11],ylim=[-1,11])
plot_trajectory!(res_i.X,width=2,color=:yellow,label="Ipopt",linestyle=:dash,xlim=[-1,11],ylim=[-1,11])
...
plot_stat("runtime",group,legend=:bottomright,["rk3","ipopt"],title="Escape")
plot_stat("iterations",group,legend=:bottom,["rk3","ipopt"],title="Escape")
