...
function quad_load_dynamics(ẋ,x,u,params)
    ...
    x_load = view(x,14:19)
    u_load = view(u,5:7)/params_load.mass

    quadrotor_lift_dynamics!(ẋ_quad, x_quad, u_quad, params_quad)
    Dynamics.double_integrator_3D_dynamics!(ẋ_load, x_load, u_load, params_load)
end
...
lift_params = (mass=0.850,
             ...
             kf=1.0,
             km=0.0245)
load_params = (mass=0.350, gravity=SVector(0,0,-9.81))
slung_load_params = (lift=lift_params, load=load_params, gravity=SVector(0,0,-9.81))

n = 13+6
m = 4+3
quad_load_model = Model(quad_load_dynamics, n, m, slung_load_params)
