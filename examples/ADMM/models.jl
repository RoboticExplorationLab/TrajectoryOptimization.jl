...
function gen_lift_model_initial(xload0,xlift0)
    ...
    dic = Dict{Symbol,Any}(:quat=>4:7)
    mm = Model(quadrotor_lift_dynamics!,13,5,quad_params,dic)
    mm.info[:radius] = 0.275  # keep 2m distance between quads
    mm
end
...
function gen_lift_model(X_load,N,dt)
      ...
      for k = 1:N-1
        ...
        dic = Dict{Symbol,Any}(:quat=>4:7)
        mm = midpoint(Model(quadrotor_lift_dynamics!,13,5,quad_params,dic),dt)
        mm.info[:radius] = 0.275  # keep 2m distance between quads
        push!(model,mm)
    end
    model
...
function gen_load_model_initial(xload0,xlift0,d)
      ...
      mm.info[:mass] = 0.35
      mm.info[:rope_length] = d
      mm
end

function gen_load_model(X_lift,N,dt,d)
      model = Model[]
      for k = 1:N-1
...
# quadrotor_lift.info[:radius] = 0.275  # keep 2m distance between quads
