...
function gen_prob_all(lift_params, load_params, r0_load=[0,0,0.5]; quat=false, num_lift=3, N=51,
    ...
    _cyl = door_obstacles(r_cylinder, goal_distance/2)
    ...
    elseif agent ∈ 1:num_lift
        ...
        Q_mid_lift = Diagonal(q_lift_mid)
        obj_mid_lift = LQRCost(Q_mid_lift, R_lift, xliftmid[agent], u0_lift[agent])
        obj_lift.cost[Nmid] = obj_mid_lift

        # Constraints
        bnd = BoundConstraint(n_lift, m_lift, u_min=u_min_lift, u_max=u_max_lift,
            x_min=x_min_lift, x_max=x_max_lift)
