...
solver = gen_quad_maze(UnitQuaternion{Float64,IdentityMap}, costfun=:Quadratic)
solver = gen_quad_maze(RPY{Float64}, use_rot=false)
solver = gen_quad_maze(UnitQuaternion{Float64,CayleyMap}, costfun=:QuatLQR)
...
# opts_al.penalty_initial = 10
# opts_al.penalty_scaling = 10
opts_al.cost_tolerance_intermediate = 1e-2
solver = gen_quad_maze(UnitQuaternion{Float64,IdentityMap}, costfun=:Quadratic, use_rot=false)
solver = gen_quad_maze(UnitQuaternion{Float64,IdentityMap}, costfun=:Quadratic, use_rot=:slack, normcon=true)
solver = gen_quad_maze(UnitQuaternion{Float64,IdentityMap}, costfun=:Quadratic, use_rot=false, normcon=true)
# solver = gen_quad_maze(RPY{Float64}, use_rot=false)
solver = gen_quad_maze(UnitQuaternion{Float64,CayleyMap}, costfun=:QuatLQR)
Z0 = deepcopy(get_trajectory(solver))
...
comp = in([:IdentityMap, :RPY, :CayleyMap])

############################################################################################
# NEW SIMPLIFIED PLOTS
############################################################################################
...
df = DataFrame(qdata_maze)
df.rots = string.(short_names.(df.name))
sub = df[in([:Quat,:ReNorm,:NormCon,:QuatSlack,:CayleyMap,:RPY]).(df.name),:]
