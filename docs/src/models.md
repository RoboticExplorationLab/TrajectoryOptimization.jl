...
It's often useful to test out the dynamics or it's Jacobians. We must pre-allocate the arrays
```julia
xdot = zeros(n)
Z = zeros(n,m)
```
...
```
or call them directly (not recommended)
```julia
model.f(xdot,x,u)
model.∇f(Z,x,u,dt)
```

If we created a partitioned Jacobian using `BlockMatrix(model)`, we can access the different pieces
```julia
fdx = Z.x   # ∂f/∂x
fdu = Z.u   # ∂f/∂u
fdt = Z.dt  # ∂f/∂dt
