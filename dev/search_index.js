...

{
    ...
    "category": "section",
    "text": "Each Constraint contains the following fields:c: the in-place constraint function. Methods dispatch over constraint functions of the form c(v,x,u) and c(v,x).\n∇c: the in-place constraint jacobian function defined as ∇c(Z,x,u) where Z is the p × (n+m) concatenated Jacobian. Methods also dispatch over constraint jacobians of the form ∇c(Z,x) where Z is p x n.\np: number of elements in the constraint vector\nlabel: a Symbol for identifying the constraintLet\'s say we have a problem with 3 states and 2 controls, the following constraints x_1^2 + x_2^2 - 1 leq 0, x_2 + u_1  = 0, and x_3 + u_2 = 0. These two constraints could be created as follows:n,m = 3,2\n\n# Inequality Constraint\nc(v,x,u) = v[1] = x[1]^2 + x[2]^2 - 1\np1 = 1\ncon = Constraint{Inequality}(c, n, m, p1, :mycon1)\n\n# Equality Constraint\nc_eq(v,x,u) = v[1:2] = x[2:3] + u\np2 = 2\ncon_eq = Constraint{Equality}(c_eq, n, m, p2, :mycon2)Here we let the constructor build the Jacobians using ForwardDiff. We can alternatively specify them explicitly:∇c(V,x,u) = begin V[1,1] = 2x[1]; V[1,2] = 2x[2]; end\ncon = Constraint{Inequality}(c, ∇c, n, m, p1, :mycon1)\n\n∇c_eq(V,x,u) = begin V[1,2] = 1; V[1,4] = 1;\n                     V[2,3] = 1; V[2,5] = 1; end\ncon_eq = Constraint{Equality}(c_eq, ∇c_eq, n, m, p2, :mycon2)We can also build a terminal constraint that only depends on the terminal state, x_N. Let\'s say we have a terminal constraint x1 + x2 + x3 = 1.A stage-wise constraint can be created with either of the two constructorsConstraint{S}(c::Function,∇c::Function,p::Int,label::Symbol) where S<:ConstraintType\nConstraint{S}(c::Function,n::Int,m::Int,p::Int,label::Symbol) where S<:ConstraintTypeThe first is the default constructor. c must be in-place of the form c(v,x,u) where v holds the constraint function values.The second will use ForwardDiff to generate the constraint Jacobian, so requires the size of the state and control input vectors."
},
...

{
    ...
    "category": "section",
    "text": "A ConstraintSet is simply a vector of constraints, and represents a set of constraints at a particular time step. There are some convenient methods for creating and working with constraint sets.Let\'s say we have a constraint with"
},
