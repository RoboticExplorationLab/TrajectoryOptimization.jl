...

{
    ...
    "category": "section",
    "text": "Augmented Lagrangian cost functions transform a constrained problem to an unconstrained problem by minimizing a cost function of the form J(XU) + λ^T c(XU) + frac12 c(XU)^T I_mu c(XU)\n \nwhere X and U are state and control trajectories J(XU) is the original cost function c(XU) is the vector-value constraint function μ is the penalty parameter and I_mu is a diagonal matrix that whose entries are μ for active constraints and 0 otherwise\n\nLets say we use our quadratic cost function we created for the pendulum bound the control inputs between 5 and use a terminal equality constraint for reaching the goal Well then form an AugmentedLagrangianCostjulia"
},

{
    "location": "costfunctions/#Create-constraints-1",
    "page": "2. Setting up a Cost Function",
    "title": "Create constraints",
    "category": "section",
    "text": "bnd = boundconstraint(n,m,umin=-5,umax=5)     # bound constraint on the controls term = goalconstraint(n,xf)                     # terminal goal constraint constraints = [bnd,term]                         # form a ConstraintSet"
},

{
    "location": "costfunctions/#Create-Cost-function-1",
    "page": "2. Setting up a Cost Function",
    "title": "Create Cost function",
    "category": "section",
    "text": "N = 21  # Number of knot points (need to initialize the trajectories) alcost = AugmentedLagrangianCost(cost, constraints, N) alcost = AugmentedLagrangianCost(cost, constraints, N, μinit=10., λinit=1.)When creating the cost function, you can initialize the penalty parameter to a specific value, as well as initialize the Lagrange multipliers to a value (which is typically zero). You can also optionally use the default inner constructor, but requires initializing all the needed variables. See source code of the previous constructor for an example.\n\nIf you want to specify the Lagrange multipliers (say from a previous solve), you can use the following constructorjulia alcost = AugmentedLagrangianCost(cost, constraints, λ, μ_init=1.)where the keyword argument for the initial penalty term is obviously optional. λ must be a `PartedVecTrajectory`, or a vector of partitioned vectors generated by [PartedArrays](https://github.com/bjack205/PartedArrays.jl). Examples for creating these can be found in [# 4. Add Constraints](@ref).\n\nTypically augmented Lagrangian cost functions are formed as part of a solve with the [AugmentedLagrangianSolver](@ref). The most convenient way to create one is from a [Problem](@ref) and an [AugmentedLagrangianSolver](@ref).julia model = Dynamics.pendulum[1] dt = 0.1 prob = Problem(model, costfun, N, dt)  # see Documentation for more way of creating a problem solver = AugmentedLagrangianSolver(prob) alcost = AugmentedLagrangianCost(prob,solver)Note that this will not create new memory for arrays stored in alcost, they will simply point to the arrays stored in the solver.\n@docs AugmentedLagrangianCost\n## Generic Costs\nFor general, non-linear cost functions use [GenericCost](@ref). Generic cost functions must define their second-order Taylor series expansion, either automatically using `ForwardDiff` or analytically.\n\nLet\'s say we wanted to use the nonlinear cost function for the pendulummath cos(\\thetaN) + \\omegaN^2 \\sum{k=1}^{N-1} cos(\\thetak) + uk^T R + uk + Q ω^2which is small when θ = π, encouraging swing-up.\n\nWe define the cost function by defining ℓ(x,u) and ``\\ell_f(x)``julia"
},

{
    "location": "costfunctions/#Define-the-stage-and-terminal-cost-functions-1",
    "page": "2. Setting up a Cost Function",
    "title": "Define the stage and terminal cost functions",
    "category": "section",
    "text": "function mycost(x,u)     R = Diagonal(0.1I,1)     Q = 0.1     return cos(x[1] + u\'Ru + Qx[2]^2) end function mycost(xN)     return cos(xN[1]) + xN[2]^2 end"
},

{
    "location": "costfunctions/#Create-the-nonlinear-cost-function-1",
    "page": "2. Setting up a Cost Function",
    "title": "Create the nonlinear cost function",
    "category": "section",
    "text": "nlcost = GenericCost(mycost,mycost,n,m)This will use `ForwardDiff` to generate the gradient and Hessian needed for the 2nd order expansion.\n\nPerformance-wise, it will be faster to specify the Jacobian analytically (which could also use `ForwardDiff` for part of it). We just need to define the following functions\n* `hess`: multiple-dispatch function of the form,\n    `Q,R,H = hess(x,u)` with sizes (n,n), (m,m), (m,n)\n    `Qf = hess(xN)` with size (n,n)\n* `grad`: multiple-dispatch function of the form,\n    `q,r = grad(x,u)` with sizes (n,), (m,)\n    `qf = grad(x,u)` with size (n,)\n\nHere\'s an example for the nonlinear cost function we used beforejulia"
},

{
    "location": "costfunctions/#Define-the-gradient-and-Hessian-functions-1",
    "page": "2. Setting up a Cost Function",
    "title": "Define the gradient and Hessian functions",
    "category": "section",
    "text": "R = Diagonal(0.1I,m) Q = 0.1 function hess(x,u)     n,m = length(x),length(u)     Qexp = Diagonal([-cos(x[1]), 2Q])     Rexp = 2R     H = zeros(m,n)     return Qexp,Rexp,Hexp end function hess(x)     return Diagonal([-cos(x[1]), 2]) end function grad(x,u)     q = [-sin(x[1]), 2Qx[2]]     r = 2Ru     return q,r end function grad(x)     return [-sin(x[1]), 2*x[2]] end"
},

{
    "location": "costfunctions/#Create-the-cost-function-1",
    "page": "2. Setting up a Cost Function",
    "title": "Create the cost function",
    "category": "section",
    "text": "nlcost = GenericCost(mycost, mycost, grad, hess, n, m) ```"
},
